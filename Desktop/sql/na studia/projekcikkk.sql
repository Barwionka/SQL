...
INSERT INTO przelewy_bankowe (numer_konta_nadawcy, numer_konta_odbiorcy, kwota, waluta) VALUES (3456789012345678901234567, 22222222222222222222222211, 5000.00, 'PLN' );
SELECT saldo FROM konta WHERE id_konta = 1;
...
select * from przelewy_bankowe p
select * from konta k
...
-- sprawdzenie
CALL aktualizuj_dane_osobowe(4,'Krzysztof','Korzeniewski','ul. Nowa 10, Warszawa','600123456','k.kor@hotmail.com');
...
CALL aktywacja_lokaty(2, 3000.00, 3.00, '2024-01-01', '2025-01-01');
select * from lokaty
...
select * from historia order by id_historia desc ;
select * from kredyty;

SELECT * FROM lokaty;
select * from historia where id_historia > 10;
...
call aktywacja_lokaty(1, 90.00, 5.00, 3, '2024-01-01', '2025-01-01')
select * from lokaty
select * from historia where id_konta = 1
...
CREATE INDEX idx_przelewy_id_konta_nadawcy ON przelewy_bankowe (id_konta_nadawcy);
CREATE INDEX idx_przelewy_id_konta_odbiorcy ON przelewy_bankowe (id_konta_odbiorcy);
...
SELECT 
    NULL AS id_przelewu,
    k_nad.numer_konta AS konto_nadawcy,
    pm.numer_konta_odbiorcy AS konto_odbiorcy,
    pm.kwota,
    'PLN' AS waluta, -- Jeśli waluta jest inna, należy dostosować
    NULL AS data_przelewu, -- Jeśli dostępne, dodaj pole data_przelewu do tabeli przelewy_miedzybankowe
    'Zewnętrzny' AS typ_przelewu
FROM przelewy_miedzybankowe pm
LEFT JOIN konta k_nad ON pm.numer_konta_nadawcy = k_nad.numer_konta;

select * from szczegoly_przelewow sp 
...
select * from przelewy_bankowe pb 
select * from historia
select * from konta
select * from lokaty 
